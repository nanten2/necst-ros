...
p.add_argument('--integtime', type=float,
               help='Integration time (sec). default=%.1f'%(integtime))
p.add_argument('--ttl_time', type=int,
               help='total time. default=%d'%(ttl_time))
p.add_argument('--memo', type=str,
               help='Working memo. This will be include in directroy name.')
args = p.parse_args()

if args.integtime is not None: integtime = args.integtime
if args.ttl_time is not None: ttl_time = args.ttl_time
...
for ch in range(1, 17):  # for each channel
    ...
    for i in range(1, int(repeat/2)):  # for each iteration
        ...
        sigma1 = 1. / (2 * (repeat - 2 * i) * (integtime * i) ** 2) * sum1
        sigma2 = 1. / (2 * (repeat - 2 * i) * (integtime * i) ** 2) * sum2
...
print("P ATT           : IF1 "+str(att_status[0])+", IF2 "+str(att_status[1])+" [dB]")
print("++++++")
