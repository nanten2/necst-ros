...
p.add_argument('--planet', type=str,
               help='target planet name', required=True)
args = p.parse_args()
...
con.obs_status(active=True, obsmode="Opt_cross",obs_script=__file__, obs_file="star", target=planet)
while num < obs_n:
    p_n = 0
    while p_n < point_n:
        off_x = 0
        ...
        print('tracking OK')
        con.obs_status(active=True, current_num=num*obs_n+p_n, current_position="ON")
        print('get spectrum...')
        ctime = dt.utcnow()
...
import opt_analy
opt_analy.opt_plot([savedir], savefig=True, figname=savedir.split("/")[-2], interactive=True)
con.obs_status(active=False)
