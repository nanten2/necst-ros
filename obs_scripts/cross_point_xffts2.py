...
while num < n:
    ...
    while p_n < point_n:
        ...
        if _now > latest_hottime + 60 * obs["load_interval"]:
            ...
            con.xffts_publish_flag(
                scan_num=num + 1, obs_mode="HOT", lamdel=off_x, betdel=off_y
            )
            time.sleep(integ)
            con.xffts_publish_flag()
            ...
            if latest_hottime > _now:
                ...
                temp = float(status.CabinTemp1)  # + 273.15
                con.xffts_publish_flag(
                    scan_num=num + 1, obs_mode="OFF", lamdel=off_x, betdel=off_y
                )
                time.sleep(integ)
                con.xffts_publish_flag()
        ...
        temp = float(status.CabinTemp1)  # + 273.15
        con.xffts_publish_flag(
            scan_num=num + 1, obs_mode="ON", lamdel=off_x, betdel=off_y
        )
        time.sleep(integ)
        con.xffts_publish_flag()
...
con.pub_loggerflag("")
logger.obslog(
    "Observation End : observation time : {:.2f\
} [min]".format(
...
log.info(
    "Observation End : observation time : {:.2f} [mi\
n]".format(
        (time.time() - start_time) / 60
    )
)
