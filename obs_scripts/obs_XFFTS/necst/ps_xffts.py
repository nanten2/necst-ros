...
dirname = "n{}_{}_{}_otf_{}".format(now.strftime('%Y%m%d%H%M%S'), obs['molecule_1'],
                                    obs['transiti_1'].split('=')[1], obs['object'])
...
con.obs_status(active=True, obsmode=obs["obsmode"], obs_script=__file__, obs_file=obsfile, target=obs["object"], num_on=obs["nON"],
...
while num < n:
    ...
    if _now > latest_hottime+60*obs['load_interval']:
        ...
        dp1 = dp.set_track(lambda_on, beta_on, vlsr, coordsys, lamdel_on, betdel_on, dcos, cosydel,
                           integ_off*2, obs['restfreq_1'] /
                           1000., obs['restfreq_2']/1000.,
    ...
    con.onepoint_move(lambda_on, beta_on, coordsys, off_x=lamdel_on,
                      off_y=betdel_on, offcoord=cosydel, dcos=dcos)
...
crpix1_1 = 8191.5 - obs['vlsr']/cdelt1_1 - \
    (500-obs['if3rd_freq_1'])/0.061038881767686015
...
crpix1_2 = 8191.5 - obs['vlsr']/cdelt1_2 - \
    (500-obs['if3rd_freq_2'])/0.061038881767686015
