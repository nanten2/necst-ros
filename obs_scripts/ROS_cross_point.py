...
p.add_argument("--obsfile", type=str, help="absolute path for obsfile", required=True)
p.add_argument("--tau", type=float, help="tau. default=%.1f" % (tau))
p.add_argument("--integmin", type=int, help="integrange_min")
p.add_argument("--integmax", type=int, help="integrange_max")
p.add_argument("--plot_mode", type=str, help="plot mode : plot/savefig")
p.add_argument("--savepath", type=str, help="save path")
args = p.parse_args()
...
if args.tau is not None:
    tau = args.tau
if args.integmin is not None:
    integmin = args.integmin
if args.integmax is not None:
    integmax = args.integmax
...
dirname = "n%s_%s_%s_cross_%s_pointing" % (
    timestamp,
    obs["molecule_1"],
...
con.obs_status(
    ...
    xgrid=obs["xgrid"],
    ygrid=obs["ygrid"],
    exposure_hot=obs["exposure"],
    exposure_off=obs["exposure"],
...
while num < n:
    ...
    while p_n < point_n:
        ...
        con.onepoint_move(
            ...
            dec,
            obs["coordsys"],
            off_x=off_x + obs["offset_Az"],
            off_y=off_y + obs["offset_El"],
            offcoord=cosydel,
            dcos=1,
        )
        print("moving...")
        ...
        if _now > latest_hottime + 60 * obs["load_interval"]:
            ...
            while status.Current_Hot != "IN":
                ...
                time.sleep(0.5)
            con.obs_status(
                active=True, current_num=num * obs["N"] + p_n, current_position="HOT"
            )
        ...
        con.obs_status(
            active=True, current_num=num * obs["N"] + p_n, current_position="OFF"
        )
        ...
        con.obs_status(
            active=True, current_num=num * obs["N"] + p_n, current_position="ON"
        )
...
f1 = os.path.join(
    savedir,
    "n%s_%s_%s_cross_%s_pointing.fits"
    % (timestamp, obs["molecule_1"], obs["transiti_1"].split("=")[1], obs["object"]),
)
f2 = os.path.join(
    savedir,
    "n%s_%s_%s_cross_%s_pointing.fits"
    % (timestamp, obs["molecule_2"], obs["transiti_2"].split("=")[1], obs["object"]),
)
...
pointing_line.analysis(
    f1,
    integ_mi=integmin,
    integ_ma=integmax,
    plot_mode=plot_mode,
    savepath_filename=save_path + "/result_cross_point{}.png".format(timestamp),
)  # f2?
