...
while num < n:
    ...
    while (
        abs(
            now.Current_M2
            - (start_m2.Current_M2 + (-20 * int(n / 2)) / 1000 + dist * num / 1000)
        )
        > 0.03
    ):  # check
        print("moving m2...")
    ...
    if _now > latest_hottime + 60 * obs["load_interval"]:
        ...
        # P_hot = numpy.sum(d1)
        # tsys_list.append(0)
        # _ 2NDLO_list1.append(dp1[3]['sg21']*1000)
    ...
    con.onepoint_move(lambda_on, beta_on, coordsys)  # ON
