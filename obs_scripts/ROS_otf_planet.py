...
dirname = "n%s_%s_%s_otfplanet_%s" % (
    timestamp,
    obs["molecule_1"],
...
if direction == 0:
    ...
    con.obs_status(
        ...
        xgrid=dx,
        ygrid=gridy,
        exposure_hot=integ_off,
        exposure_off=integ_off,
        exposure_on=integ_on,
        scan_direction="x",
    )
elif direction == 1:
    dx = 0
...
while rp_num < rp:
    while num < n:
        ...
        if 1:  # try:#デバッグ用
            ...
            con.planet_move(
                planet, off_x=ssx, off_y=ssy, offcoord=cosydel, dcos=offset_dcos
            )
            ...
            print("move ON")
            con.obs_status(
                active=True, current_num=scan_point * num, current_position="ON"
            )
            delay = 3
            ctime = time.time()
            ...
            d = con.oneshot_achilles(
                repeat=scan_point, exposure=integ_on, stime=start_on
            )
            print("getting_data...")

            while start_on + obs["otflen"] / 24.0 / 3600.0 > 40587 + time.time() / (
                24.0 * 3600.0
            ):
                time.sleep(0.001)
