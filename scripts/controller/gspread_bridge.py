...
def read():
     ...
     while not rospy.is_shutdown():
          ...
          try:
               st = time.time()
               data1=db.SELECT("Node_status", date, date)[-1]#, num=1)
               ct = time.time()-st
               a.append(ct)
               print("time", ct)
               print("average", np.average(a))
               data2 = ast.literal_eval(data1[1])
               if data1[1]:
                    data_dict1 = ast.literal_eval(data2[0])
                    func_status.update(data_dict1)
               if data1[1]:
                    node_status = ast.literal_eval(data2[1])
                    print(node_status)
               if data1[1]:
                    data_dict2 = ast.literal_eval(data2[2])
                    weather_status.update(data_dict2)
               if data1[1]:
                    data_dict3 = ast.literal_eval(data2[3])
