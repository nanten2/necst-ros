...
para_init = numpy.array([25000., 0.1, 0.0001])
    #-----
def analysis(directory, file_name, mi=5000, ma=15000, width=500, integ_mi=8000, integ_ma=9000):
# open file
...
#azel(shiotani added)
    az_list = hdu[1].data['AZIMUTH']
    el_list = hdu[1].data['ELEVATIO']
    thot = hdu[1].data['thot']
    #print(xmask)
    ...
    return {'dAz':dAz, 'dEl':dEl, 'hpbw_az':hpbw_az, 'hpbw_el':hpbw_el, 'center_az_x':center_az_x, 'center_el_x':center_el_x,'center_az_y':center_az_y, 'center_el_y':center_el_y, 'thot':thot, 'hosei':hosei_parameter}
