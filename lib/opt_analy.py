...
def process_static(dir_list, *, clip_sigma=None, clip_const=None):
    ...
    rawdata = np.concatenate(
        [np.loadtxt(os.path.join(_dir, "process.log")) for _dir in dir_list]
    )
    ...
    if clip_sigma is not None:
        ...
        d = rawdata[
            (abs(rawdata[:, ind_dx] - rawdx_med) < x_clip * rawdx_std)
            & (abs(rawdata[:, ind_dy] - rawdy_med) < y_clip * rawdy_std)
        ]
    elif clip_const is not None:
        x_clip, y_clip = clip_const
        d = rawdata[
            (abs(rawdata[:, ind_dx] - rawdx_med) < x_clip)
            & (abs(rawdata[:, ind_dy] - rawdy_med) < y_clip)
        ]
    else:
        d = rawdata
...
def opt_plot(
    dir_list, *, clip_sigma=(3.0, 3.0), savefig=True, figname=None, interactive=False
):
    ...
    for _d in d_list:
        ...
        _dx = _d[:, ind_dx] * np.cos(np.radians(-1.76)) - _d[:, ind_dy] * np.sin(
            np.radians(-1.76)
        )
        _dy = _d[:, ind_dx] * np.sin(np.radians(-1.76)) + _d[:, ind_dy] * np.cos(
            np.radians(-1.76)
        )
        _d = _d[
            (abs(_dx - dx_avg) < dx_std * clip_sigma[0])
            & (abs(_dy - dy_avg) < dy_std * clip_sigma[1])
        ]
    ...
    [
        [
            _a.scatter(_d[:, int(_xind)], _d[:, int(_yind)], marker=".")
            for _a, _xind, _yind in zip(ax, axes[:, 0], axes[:, 1])
        ]
        for _d in d_list
    ]
    ...
    row_labels = [" dx ", " dy "]
    tbl_vals = [
        ["{:.2e}".format(dx_avg), "{:.2f}".format(dx_std)],
        ["{:.2e}".format(dy_avg), "{:.2f}".format(dy_std)],
    ]

    tbl.table(
        cellText=tbl_vals, rowLabels=row_labels, colLabels=col_labels, loc=tbl_loc
    )
...
def fdx(altaz, dAz, de, chi, omega, eps):
    ...
    El = np.deg2rad(El)
    dx = (
        chi * np.sin(np.deg2rad(omega - Az)) * np.sin(El)
        + eps * np.sin(El)
        + dAz * np.cos(El)
        + de
    )
...
def opt_fit(
    ...
    *,
    ...
    dx_avg, dy_avg, dx_std, dy_std, *_ = process_static(dir_list, clip_sigma=(3.0, 3.0))
    d_list_noclip = [
        np.loadtxt(os.path.dirname(_dir + "/") + "/for_fit.log") for _dir in dir_list
    ]
    d_list = [
        _d[
            (abs(_d[:, ind_dx] - dx_avg) < dx_std * 3.0)
            & (abs(_d[:, ind_dy] - dy_avg) < dy_std * 3.0)
        ]
        for _d in d_list_noclip
    ]
    ...
    back_list = [
        ...
        + np.array(
            [
                fdx(_d[:, (ind_az, ind_el)].T, dAz, de, chi_x, omega_x, eps),
                fdy(_d[:, (ind_az, ind_el)].T, dEl, chi_y, omega_y, g1),
            ]
        ).T
        for _d in d_list
    ]
    ...
    # fitting
    # p0が初期期値
    p_opt_dx, coval_dx = curve_fit(
        fdx,
        np.concatenate(d_list)[:, (ind_az, ind_el)].T,
        back[:, 0],
        p0=np.array([dAz, de, chi_x, omega_x, eps]),
    )
    p_opt_dy, coval_dy = curve_fit(
        fdy,
        np.concatenate(d_list)[:, (ind_az, ind_el)].T,
        back[:, 1],
        p0=np.array([dEl, chi_y, omega_y, g1]),
    )
    print(p_opt_dx)
    ...
    res_list = [
        ...
        - np.array(
            [
                fdx(_d[:, (ind_az, ind_el)].T, dAz_n, de_n, chi_xn, omega_xn, eps_n),
                fdy(_d[:, (ind_az, ind_el)].T, dEl_n, chi_yn, omega_yn, g1_n),
            ]
        ).T
        for _b, _d in zip(back_list, d_list)
    ]
    ...
    [
        [
            _a.scatter(_d[:, int(_xind)], _r[:, int(_yind)], marker=".")
            for _a, _xind, _yind in zip(ax[0:4], axes[:4, 0], axes[:4, 1])
        ]
        for _d, _r in zip(d_list, res_list)
    ]
    ...
    row_labels = [" dx ", " dy "]
    tbl_vals = [
        ["{:.2e}".format(res_dx_avg), "{:.2f}".format(res_dx_std)],
    ...
    hosei_print = [
        "| {} | {} | {} | {} | {:.3%} |".format(_nm, _ih, _oh, _oh - _ih, _uct)
        for _nm, _ih, _oh, _uct in zip(
            param_names, hosei[:16], hosei_op[:16], hosei_uct
        )
    ]
    ...
    f.write(
        "number of cliped stars :\t{}\n".format(
            ",\t".join(
                [str(len(_dc) - len(_d)) for _dc, _d in zip(d_list_noclip, d_list)]
            )
        )
    )
