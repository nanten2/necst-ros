...
class antenna_move(object):
    ...
    def start_thread(self):
        ...
        th2.setDaemon(True)
        th2.start()
        th3 = threading.Thread(target = self.mytime)
        th3.setDaemon(True)
        th3.start()
        th4 = threading.Thread(target = self.test)
        th4.setDaemon(True)
        th4.start()
    ...
    def mytime(self):
        ...
        while not rospy.is_shutdown():
            if not self.mytime_flag:
                self.now = time.time()
                self.i = 0
            self.now = time.time()+self.i*0.001                
            self.i += self.speed
            rate.sleep()
            #print(self.now-time.time())
        return
    ...
    def calc_pid(self, az_arcsec, el_arcsec, az_max_rate, el_max_rate):
        ...
        self.t1 = self.now
        print("t1, t_az", self.t1, self.t_az)
    ...
    def pub_status(self):
        ...
        while not rospy.is_shutdown():
            ...
            status.timestamp = self.command_time#self.now
            #publisher2
            #----------
            if self.task:
                task.data = True
...
if __name__ == '__main__':
    ...
    rospy.Subscriber('accele', Float64_necst, ant.accele, queue_size=1)    
    rospy.spin()    
