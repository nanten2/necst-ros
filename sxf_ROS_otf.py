...
while rp_num < rp:
    ...
    while num < n: 
        ...
        if _now > latest_hottime+60*obs['load_interval']:
            ...
            con.xffts_publish_flag(1, path_to_db, str(num), "HOT")
            time.sleep(integ_off)
            con.xffts_publish_flag(0, path_to_db, str(num), "OFF")
            """
            d1 = d['dfs1'][0]
            d2 = d['dfs2'][0]
            d1_list.append(d1)
        ...
        temp = float(status.CabinTemp1)
        #d = con.oneshot_achilles(exposure=integ_off)
        con.xffts_publish_flag(1, path_to_db, str(num), "OFF")
        time.sleep(integ_off)
        con.xffts_publish_flag(0, path_to_db, str(num), "OFF")
        """
        d1 = d['dfs1'][0]
        ...
        con.xffts_publish_flag(1, path_to_db, str(num), "ON")
        ...
        while start_on + (obs['otflen']+rampt)/24./3600. > 40587 + time.time()/(24.*3600.):
            ...
            time.sleep(0.001)
        con.xffts_publish_flag(0, path_to_db, str(num), "ON")
